...
bool VirtualCameraProgramSink::start(const QString &streamName) {
    ...
    m_impl->yuyvBuffer.resize(static_cast<qsizetype>(m_impl->width) * m_impl->height * 2);
    m_active = true;
...
void VirtualCameraProgramSink::submitFrame(const QImage &frame) {
    ...
    if (m_impl->yuyvBuffer.size() != expected)
        m_impl->yuyvBuffer.resize(expected);

    rgbaToYuyv(rgba.constBits(), m_impl->width, m_impl->height, rgba.bytesPerLine(),
               m_impl->yuyvBuffer.data());

    const ssize_t written = ::write(m_impl->fd, m_impl->yuyvBuffer.constData(),
                                    static_cast<size_t>(m_impl->yuyvBuffer.size()));
